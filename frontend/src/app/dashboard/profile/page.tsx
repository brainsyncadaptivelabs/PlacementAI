...
import { ChevronRight, Bell, Shield, Download, Trash2, Key, Loader2 } from "lucide-react";
            ...
            <CardContent className="p-8 space-y-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                     ...
                     <Input id="full-name" defaultValue={user?.fullName || ""} className="bg-slate-50 border-none focus-visible:ring-primary/20" />
                  </div>
                  <div className="space-y-2">
                     <Label htmlFor="email" className="text-xs font-bold uppercase tracking-wider text-slate-500">Email Address</Label>
                     <Input id="email" defaultValue={user?.email || ""} disabled className="bg-slate-100 border-none opacity-60" />
                  </div>
                  <div className="space-y-2">
                     <Label htmlFor="college" className="text-xs font-bold uppercase tracking-wider text-slate-500">College</Label>
                     <Input id="college" defaultValue={user?.collegeName || ""} className="bg-slate-50 border-none focus-visible:ring-primary/20" />
                  </div>
                  <div className="space-y-2">
                     <Label htmlFor="branch" className="text-xs font-bold uppercase tracking-wider text-slate-500">Branch</Label>
                     <Input id="branch" defaultValue={user?.branch || ""} className="bg-slate-50 border-none focus-visible:ring-primary/20" />
                  </div>
                  <div className="space-y-2">
                     <Label htmlFor="grad-year" className="text-xs font-bold uppercase tracking-wider text-slate-500">Graduation Year</Label>
                     <Input id="grad-year" defaultValue={user?.graduationYear?.toString() || ""} className="bg-slate-50 border-none focus-visible:ring-primary/20" />
                  </div>
               </div>
