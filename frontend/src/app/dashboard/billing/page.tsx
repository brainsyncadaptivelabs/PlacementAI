    ...
    <div className="p-4 md:p-8 space-y-16 max-w-6xl mx-auto select-none">
      ...
      <div className="space-y-6 pt-12 border-t border-border">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-black font-heading">Compare Plan Features</h2>
          <p className="text-muted-foreground text-xs font-medium">Detailed side-by-side matrices of all included functions.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse bg-card">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                <th className="p-4 text-xs font-black uppercase tracking-widest text-muted-foreground">Feature</th>
                {getActivePlans().map((p) => (
                  <th key={p.id} className="p-4 text-xs font-black uppercase tracking-widest text-muted-foreground text-center">
                    {p.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {getActiveComparison().map((row, idx) => (
                <tr key={idx} className="border-b border-border hover:bg-muted/10">
                  <td className="p-4 text-xs font-bold text-foreground">{row.feature}</td>
                  {row.values.map((val, valIdx) => (
                    <td key={valIdx} className="p-4 text-xs font-medium text-center">
                      {val === "✔" ? (
      ...
      <div className="space-y-8 pt-12 border-t border-border">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-black font-heading">Loved by Candidates & Recruiters</h2>
          <p className="text-muted-foreground text-xs font-medium">See how PlacementAI drives measurable results.</p>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          {[
            {
              quote: "PlacementAI predicted my compatibility score for Google perfectly, helping me target AWS gaps before interviews.",
              author: "Nikhil Sharma",
              role: "Student, IIT Bombay",
              rating: 5
            },
            {
              quote: "Our screening timeline dropped from 3 weeks to 2 days thanks to AI Resume Ranking. Absolutely incredible tool.",
              author: "Priyah Patel",
              role: "Talent Acquisition, InnovateCorp",
              rating: 5
            },
            {
              quote: "We consolidated 1500+ student data points onto a single live predictive metrics dashboard. Amazing visibility.",
              author: "Dr. A. K. Sen",
              role: "TPO Director, Delhi Tech University",
              rating: 5
            }
          ].map((test, index) => (
            <Card key={index} className="p-6 bg-card border border-border flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <p className="text-xs italic leading-relaxed text-muted-foreground">"{test.quote}"</p>
              </div>
      ...
      <div className="flex flex-col items-center justify-center gap-4 text-center border-t border-border pt-12">
        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Secure Checkout & Transactions</p>
        <div className="flex flex-wrap items-center justify-center gap-6 opacity-60">
          {["Razorpay", "Stripe", "UPI", "Visa", "Mastercard"].map((logo) => (
            <span key={logo} className="bg-muted text-muted-foreground text-[10px] font-black px-3 py-1 rounded-md uppercase tracking-wider border border-border shadow-sm">
              {logo}
            </span>
          ))}
        </div>
      ...
      <div className="space-y-8 pt-12 border-t border-border">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground text-xs font-medium">Common details about our payments, cancellation, and plans.</p>
        </div>
          ...
          {[
            {
              ...
              a: "Yes! There are no lock-in periods or commitments. You can cancel your monthly or yearly plan directly inside the dashboard with one click."
            },
            {
              q: "How does the annual plan discount work?",
              a: "When selecting the yearly billing cycle, we calculate a 20% discount compared to 12 months of individual payments and bill that single discounted amount once."
            },
            {
              q: "What payment methods do you accept?",
              a: "We support all major payment types including credit/debit cards, UPI, Google Pay, Net Banking, and popular wallets through our secure checkout partners."
            },
            {
              q: "Can I switch roles or upgrade later?",
              a: "Yes, you can upgrade, downgrade, or switch billing periods instantly. The system calculates and prorates remaining credits to your new plan selection."
            }
          ].map((faq, index) => (
            <div key={index} className="space-y-2 p-5 bg-card border border-border rounded-2xl hover:border-primary/20 transition-all duration-300">
