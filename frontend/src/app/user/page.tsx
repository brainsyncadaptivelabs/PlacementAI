    ...
    <div className="min-h-screen bg-[#f8fafc] p-8 space-y-8 font-sans">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-slate-900 rounded-[2rem] p-8 lg:p-12 text-white relative overflow-hidden shadow-2xl shadow-slate-200">
         <div className="relative z-10 space-y-6 lg:max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10 text-primary text-[10px] font-black uppercase tracking-widest">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
               ...
               <Button variant="outline" className="border-white/20 bg-white/5 hover:bg-white/10 text-white font-black px-8 h-14 rounded-2xl backdrop-blur-sm text-md">
                  Update Resume
               </Button>
            </div>
         </div>
         ...
         <div className="relative z-10 flex flex-col items-center gap-4 bg-white/5 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 shadow-inner lg:w-72">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <Card className="border-none shadow-sm bg-white p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="flex -space-x-3 mb-6">
                     {['G', 'A', 'M', 'A'].map((l, i) => (
                        <div key={i} className="w-10 h-10 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center font-black text-xs text-slate-400 shadow-sm">{l}</div>
                     ))}
            ...
            <Card className="border-none shadow-sm bg-white overflow-hidden">
               <CardHeader className="px-8 py-6 border-b border-slate-50 flex flex-row items-center justify-between">
                  ...
                  <Badge variant="outline" className="rounded-lg py-1.5 px-3 font-bold border-slate-200">60% Complete</Badge>
               </CardHeader>
                  ...
                  <div className="relative space-y-12">
                     ...
                     {[
                        ...
                        <div key={i} className="relative pl-12 group">
                           <div className={`absolute left-0 w-8 h-8 rounded-full border-4 border-white shadow-md flex items-center justify-center transition-all duration-300 z-10 ${step.status === 'Completed' ? 'bg-primary text-white' : step.status === 'In Progress' ? 'bg-white text-primary border-primary' : 'bg-slate-100 text-slate-400'}`}>
                           ...
                           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                              <div>
                                 ...
                                 <div className="flex flex-wrap gap-2 mt-2">
                                    {step.items.map(item => (
                                       <span key={item} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 border border-slate-100 px-2 py-0.5 rounded">{item}</span>
                                    ))}
                                 </div>
                              </div>
                              <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${step.status === 'Completed' ? 'text-emerald-500' : step.status === 'In Progress' ? 'text-primary' : 'text-slate-300'}`}>{step.status}</span>
                           </div>
                        </div>
                     ))}
                  </div>
         ...
         <div className="space-y-8">
            ...
            <Card className="border-none shadow-sm bg-white p-6">
               ...
               <div className="space-y-4">
                  {[
                     ...
                     <div key={i} className="flex gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 group cursor-pointer hover:border-primary/30 transition-all duration-300">
                        ...
                        <div className="flex-1 min-w-0">
                           ...
                           <div className="flex items-center gap-3 mt-2">
                              ...
                              <Badge className="bg-white border-slate-200 text-slate-600 text-[9px] font-black">{event.date}</Badge>
                           </div>
                        </div>
                     </div>
                  ))}
               </div>
               <Button variant="outline" className="w-full mt-6 rounded-xl text-xs font-black uppercase tracking-[0.2em] h-12 border-slate-100 hover:bg-slate-50">
                  Full Schedule
               </Button>
            </Card>
            ...
            <Card className="border-none shadow-sm bg-primary p-8 text-white relative overflow-hidden rounded-[2rem]">
               <div className="relative z-10 space-y-6">
                  ...
                  <div className="bg-white/10 border border-white/20 rounded-2xl p-4 cursor-pointer hover:bg-white/20 transition-all group">
                  ...
                  <Button className="w-full bg-white text-primary hover:bg-slate-50 font-black h-12 rounded-xl shadow-lg">
                     Open AI Chat
                  </Button>
               </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <Button variant="outline" className="h-24 flex-col gap-2 rounded-2xl bg-white border-slate-100 hover:border-primary/20 hover:text-primary transition-all group shadow-sm font-black text-xs uppercase tracking-widest">
                  <FileText className="w-6 h-6 text-slate-400 group-hover:text-primary group-hover:scale-110 transition-all" />
                  ATS Scan
               </Button>
               <Button variant="outline" className="h-24 flex-col gap-2 rounded-2xl bg-white border-slate-100 hover:border-primary/20 hover:text-primary transition-all group shadow-sm font-black text-xs uppercase tracking-widest">
                  <Map className="w-6 h-6 text-slate-400 group-hover:text-primary group-hover:scale-110 transition-all" />
                  Roadmap
               </Button>
            </div>
         </div>
