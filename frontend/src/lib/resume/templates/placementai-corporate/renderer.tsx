...
export default function CorporateRenderer({ data, previewMode = false, highlightSection = "" }: RendererProps) {
  ...
  return (
    <div 
      className={`bg-card text-foreground font-sans leading-tight selection:bg-slate-200 ${
    ...
    >
      ...
      >
        <div>
          ...
          <div className={`text-muted-foreground font-bold uppercase tracking-[0.2em] ${previewMode ? "text-[8px] mt-1" : "text-[10px] mt-2"}`}>
            Executive Professional
          </div>
        </div>
        <div className={`text-right font-bold text-muted-foreground ${previewMode ? "text-[9px] space-y-0.5" : "text-[11px] space-y-1"}`}>
          <div>{personalInfo.email}</div>
          <div>{personalInfo.phone}</div>
          <div className="text-muted-foreground/70">{personalInfo.linkedin}</div>
        </div>
      </div>
      ...
      {summary && (
        <div className={previewMode ? "mb-3" : "mb-8"} style={highlightStyle("summary")} data-block="summary">
          ...
          <p className="text-foreground text-justify leading-relaxed font-medium">{summary}</p>
        </div>
      )}
        ...
        <div className={previewMode ? "mb-3" : "mb-8"} style={highlightStyle("skills")} data-block="skills">
          ...
          <div className={`grid grid-cols-3 gap-x-4 ${previewMode ? "gap-y-1" : "gap-y-2"}`}>
            {skills.map((skill, index) => (
              <div key={index} className="text-foreground font-bold flex items-center gap-2">
            ...
            {experience.map((exp) => (
              <div key={exp.id} data-block="experience-item">
                <div className="flex justify-between items-end mb-1">
                  <div>
                    <div className={`font-black text-foreground uppercase tracking-tight ${previewMode ? "text-sm leading-tight" : "text-lg leading-none"}`}>{exp.company}</div>
                    <div className={`text-indigo-600 font-black uppercase tracking-wide ${previewMode ? "text-[10px] mt-0.5" : "text-xs mt-1"}`}>{exp.role}</div>
                  </div>
                  <div className={`font-black text-muted-foreground/70 uppercase tracking-widest ${previewMode ? "text-[9px]" : "text-[11px]"}`}>{exp.duration}</div>
                </div>
                <ul className={`mt-1.5 ${previewMode ? "space-y-1" : "space-y-2"}`}>
                  {exp.description.split("\n").filter(line => line.trim().length > 0).map((bullet, idx) => (
                    <li key={idx} className="text-foreground leading-relaxed flex items-start gap-2.5">
                      <span className="text-muted-foreground/50 mt-1.5 shrink-0 text-[8px]">■</span>
          ...
          <div className="space-y-4">
            {education.map((edu) => (
              <div key={edu.id} className="flex justify-between items-start" data-block="education-item">
                <div>
                  <div className="font-black text-foreground text-base uppercase tracking-tight">{edu.institution}</div>
                  <div className="text-muted-foreground font-bold italic">{edu.degree}</div>
                  {edu.details && <div className="text-muted-foreground text-[11px] mt-1 font-medium">{edu.details}</div>}
                </div>
                <div className="text-muted-foreground/70 font-black text-[11px] uppercase tracking-widest whitespace-nowrap ml-4">{edu.duration}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <div style={highlightStyle("certifications")} data-block="certifications">
                ...
                <ul className="space-y-1.5">
                  {certifications.map((cert, index) => (
                    <li key={index} className="text-foreground text-[11px] font-bold flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {projects.map((proj) => (
                    <div key={proj.id} data-block="projects-item">
                      <div className="font-bold text-foreground text-[11px]">{proj.name}</div>
                      <div className="text-[10px] text-muted-foreground font-bold">{proj.duration}</div>
                    </div>
                  ))}
                </div>
              </div>
