  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] p-8 space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tight font-heading">Talent Acquisition</h1>
          <p className="text-muted-foreground font-medium">Empowering your hiring process with AI-driven insights.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative hidden lg:block">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/70" />
            <input className="pl-10 pr-4 py-2.5 w-64 bg-card border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Search candidates..." />
          </div>
          <Button variant="outline" className="border-border bg-card rounded-xl font-bold h-11">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <Card className="border-none shadow-sm bg-primary text-white overflow-hidden relative group cursor-pointer">
            <CardContent className="p-6 relative z-10">
               ...
               <div className="mt-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-wider bg-card/10 w-fit px-2 py-1 rounded">
                  <Plus className="w-3 h-3" /> 2 New Today
               </div>
            </CardContent>
            <div className="absolute -right-8 -bottom-8 w-32 h-32 bg-card/10 rounded-full group-hover:scale-125 transition-transform duration-700" />
         </Card>
         ...
         <Card className="border-none shadow-sm bg-card overflow-hidden group cursor-pointer">
            <CardContent className="p-6">
               ...
               <p className="text-4xl font-black text-foreground mb-1">18%</p>
               <p className="text-xs font-bold text-muted-foreground/70 uppercase tracking-widest">Hiring Velocity</p>
               <div className="mt-4">
            ...
            <div className="p-0">
               <div className="divide-y divide-slate-50">
                  {[
                     ...
                     <div key={i} className="flex items-center justify-between p-8 hover:bg-muted/50 transition-colors cursor-pointer group">
                        <div className="flex items-center gap-5">
                           <div className="w-12 h-12 rounded-2xl bg-muted border border-border flex items-center justify-center font-black text-muted-foreground/70 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all duration-300">
                              {app.name.split(' ').map(n => n[0]).join('')}
                           </div>
                           <div>
                              <div className="flex items-center gap-2">
                                 <p className="text-sm font-black text-foreground group-hover:text-primary transition-colors">{app.name}</p>
                        ...
                        <div className="flex items-center gap-12">
                           <div className="text-center">
                              <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mb-1">AI Score</p>
                           ...
                           <div className="flex items-center gap-2">
                              <Button size="icon" variant="ghost" className="rounded-xl text-muted-foreground/70 hover:bg-card hover:shadow-md hover:text-primary transition-all">
         ...
         <div className="space-y-6">
            ...
            <Card className="border-none shadow-sm bg-card p-6">
               <div className="flex items-center justify-between mb-8">
                  <div>
                     ...
                     <p className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest">Upcoming Today</p>
                  </div>
                  <CalendarIcon className="w-5 h-5 text-muted-foreground/50" />
               </div>
               <div className="space-y-4">
                  {[1,2,3].map(i => (
                     <div key={i} className="flex gap-4 p-4 rounded-2xl bg-muted border border-border group cursor-pointer hover:border-primary/30 hover:bg-card hover:shadow-xl hover:shadow-primary/5 transition-all duration-300">
                        <div className="flex flex-col items-center justify-center px-4 py-2 bg-card rounded-xl border border-border shadow-sm shrink-0 group-hover:bg-primary group-hover:border-primary transition-colors">
                           <span className="text-[10px] font-black text-primary uppercase group-hover:text-white">May</span>
                           <span className="text-xl font-black text-foreground leading-none group-hover:text-white">2{i}</span>
                        </div>
                        <div className="flex-1 min-w-0 py-1">
                           <p className="text-xs font-bold text-foreground truncate group-hover:text-primary transition-colors">Technical Round - Candidate {i}</p>
                           <div className="flex items-center gap-2 mt-2">
                              <Clock className="w-3 h-3 text-muted-foreground/70" />
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tight">10:30 AM - 11:30 AM</span>
                           </div>
                        </div>
               ...
               <Button variant="outline" className="w-full mt-8 rounded-xl text-xs font-black uppercase tracking-[0.2em] h-12 border-border hover:bg-muted">
                  View Full Calendar
               </Button>
            </Card>
            ...
            <Card className="border-none shadow-sm bg-slate-900 p-8 text-white relative overflow-hidden rounded-3xl">
               <div className="relative z-10 space-y-6">
                  <div className="p-3 bg-card/10 rounded-2xl w-fit">
                     <Zap className="w-6 h-6 text-primary fill-primary" />
                  </div>
                  <div>
                     <h4 className="text-xl font-bold font-heading leading-tight mb-2">Smart Sourcing AI</h4>
                     <p className="text-xs text-white/60 leading-relaxed font-medium">Find students matching your tech stack instantly using our LLM-powered search.</p>
                  </div>
                  <div className="relative">
                     <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
                     <input className="w-full bg-card/5 border border-white/10 rounded-xl pl-12 pr-4 py-3.5 text-sm placeholder:text-white/20 focus:ring-2 focus:ring-primary/40 focus:bg-card/10 transition-all outline-none" placeholder="e.g. React, Go, AWS, Docker" />
                  </div>
               </div>
