...
-- Match existing users by email first to populate supabase_uuid
UPDATE users u
SET supabase_uuid = au.id
FROM auth.users au
WHERE LOWER(u.email) = LOWER(au.email) AND u.supabase_uuid IS NULL;
