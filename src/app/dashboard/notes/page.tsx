    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold font-heading text-slate-900">Notes & Study Material</h1>
          <p className="text-slate-500">Hand-picked resources to help you master placement topics.</p>
        </div>
        <div className="relative w-full max-w-sm">
           <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
           <Input placeholder="Search notes..." className="pl-10 bg-white border-slate-200" />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
         {categories.map(cat => (
            <Badge 
              ...
              className={`px-4 py-1.5 cursor-pointer text-sm font-medium transition-all ${selectedCategory === cat ? 'bg-primary text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
         {filteredNotes.map(note => (
            <Card key={note.id} className="border-none shadow-sm hover:shadow-md transition-shadow group cursor-pointer overflow-hidden bg-white">
               <CardContent className="p-6 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                     <h3 className="font-bold text-slate-900 truncate group-hover:text-primary transition-colors">{note.title}</h3>
                     <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{note.domain}</p>
                  ...
                  <button 
                    ...
                    className={`p-2 rounded-full transition-colors ${bookmarked.includes(note.id) ? 'bg-primary/10 text-primary' : 'text-slate-300 hover:bg-slate-100'}`}
                  >
